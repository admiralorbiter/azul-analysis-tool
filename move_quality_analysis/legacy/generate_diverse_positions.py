#!/usr/bin/env python3
"""
Generate Diverse Positions for Move Quality Analysis

This script generates a comprehensive set of diverse Azul positions covering:
- Different game phases (opening, middlegame, endgame)
- Various strategic scenarios (blocking, scoring, floor line management)
- Skill-level specific positions
- High-risk and edge case scenarios

This is the foundation for building robust "good move" data.
"""
...
class SkillLevel(Enum):
    """Skill levels for position generation."""
    BEGINNER = "beginner"
...
class DiversePositionGenerator:
    ...
    def _define_position_specifications(self) -> List[PositionSpecification]:
        ...
        specs.extend([
            PositionSpecification(
                ...
                risk_level=0.1,
                tags=["opening", "factory_selection", "beginner"]
            ),
            PositionSpecification(
                ...
                risk_level=0.2,
                tags=["opening", "pattern_building", "intermediate"]
            ),
            PositionSpecification(
                ...
                risk_level=0.3,
                tags=["opening", "blocking", "advanced"]
            )
        ])
        ...
        specs.extend([
            PositionSpecification(
                ...
                risk_level=0.4,
                tags=["middlegame", "blocking", "intermediate"]
            ),
            PositionSpecification(
                ...
                risk_level=0.5,
                tags=["middlegame", "scoring", "advanced"]
            ),
            PositionSpecification(
                ...
                risk_level=0.8,
                tags=["middlegame", "floor_line", "expert"]
            ),
            PositionSpecification(
                name="Risk Management Scenarios",
                description="High-risk positions requiring careful risk assessment",
                game_phase=GamePhase.MIDDLEGAME,
                strategic_scenario=StrategicScenario.RISK_MANAGEMENT,
                skill_level=SkillLevel.EXPERT,
                target_count=15,
                complexity_score=0.95,
                risk_level=0.9,
                tags=["middlegame", "risk_management", "expert"]
            )
        ])
        ...
        specs.extend([
            PositionSpecification(
                ...
                risk_level=0.6,
                tags=["endgame", "wall_completion", "advanced"]
            ),
            PositionSpecification(
                ...
                complexity_score=0.9,
                risk_level=0.7,
                tags=["endgame", "final_scoring", "expert"]
            )
        ])
    ...
    def generate_positions_for_spec(self, spec: PositionSpecification) -> List[AzulState]:
        ...
        print(f"  Skill: {spec.skill_level.value}, Complexity: {spec.complexity_score:.2f}")
        
        for i in range(spec.target_count):
            try:
                position = self._generate_position_for_spec(spec)
                if position and self._validate_position(position, spec):
    ...
    def _generate_position_for_spec(self, spec: PositionSpecification) -> AzulState:
        ...
        elif spec.strategic_scenario == StrategicScenario.RISK_MANAGEMENT:
            state = self._add_risk_management_scenario(state, spec)
    ...
    def _modify_for_opening(self, state: AzulState, spec: PositionSpecification) -> AzulState:
        ...
        # Add minimal pattern line progress
        if spec.complexity_score > 0.3:
            self._add_minimal_pattern_progress(state)
    ...
    def _add_scoring_scenario(self, state: AzulState, spec: PositionSpecification) -> AzulState:
        ...
        # Add some pattern line progress for scoring
        self._add_scoring_pattern_progress(state, spec.complexity_score)
    ...
    def _add_risk_management_scenario(self, state: AzulState, spec: PositionSpecification) -> AzulState:
        """Add risk management scenario to the position."""
        # Add high-risk floor line situation
        for player_id in range(len(state.agents)):
            agent = state.agents[player_id]
            floor_tiles = int(spec.risk_level * 7)  # 0-7 tiles
            for _ in range(floor_tiles):
                tile_type = random.randint(0, 4)
                agent.AddToFloor([tile_type])
        
        # Add tiles that create difficult choices
        for factory_id in range(min(2, len(state.factories))):
            if factory_id < len(state.factories):
    ...
    def _add_factory_control_scenario(self, state: AzulState, spec: PositionSpecification) -> AzulState:
        ...
        for factory_id in range(min(4, len(state.factories))):
            if factory_id < len(state.factories):
                ...
                tile_types = random.sample(range(5), random.randint(2, 4))
    ...
    def _add_minimal_pattern_progress(self, state: AzulState):
        """Add minimal pattern line progress."""
        for player_id in range(len(state.agents)):
            agent = state.agents[player_id]
    ...
    def _add_scoring_pattern_progress(self, state: AzulState, complexity: float):
        """Add pattern progress that creates scoring opportunities."""
        for player_id in range(len(state.agents)):
            agent = state.agents[player_id]
            lines_to_fill = int(2 + complexity * 3)  # 2-5 lines
            for _ in range(lines_to_fill):
                line_id = random.randint(0, 4)
                tile_type = random.randint(0, 4)
                tiles_to_add = min(random.randint(2, line_id + 1), line_id + 1)
                for _ in range(tiles_to_add):
                    agent.AddToPatternLine(line_id, 1, tile_type)
    
    def _validate_position(self, state: AzulState, spec: PositionSpecification) -> bool:
        """Validate that a position meets the specification requirements."""
        try:
            ...
            # Check if position matches complexity requirements
            total_tiles = sum(factory.total for factory in state.factories)
            if spec.complexity_score < 0.3 and total_tiles > 15:
                return False  # Too complex for simple spec
            
            if spec.complexity_score > 0.7 and total_tiles < 8:
                return False  # Too simple for complex spec
...
def main():
    ...
    generator = DiversePositionGenerator()
    
    # Generate all positions
    positions = generator.generate_all_positions()
    
    # Save positions to file
    output_file = "data/diverse_positions.json"
