...
function TrainingHistoryComponent({ setStatusMessage, loading, setLoading }) {
    ...
    const loadTrainingHistory = async () => {
        try {
            setLoading(true);
            const result = await getTrainingHistory(filters);
    ...
    const loadConfigurations = async () => {
        try {
            const result = await getNeuralConfigurations(configFilters);
    ...
    const handleDeleteConfiguration = async (configId) => {
        try {
            await deleteNeuralConfiguration(configId);
    ...
    const handleSaveConfiguration = async (config) => {
        try {
            setLoading(true);
            if (editingConfig) {
                await updateNeuralConfiguration(editingConfig.config_id, config);
                setStatusMessage('success', 'Configuration updated successfully');
            } else {
                await saveNeuralConfiguration(config);
    ...
    return React.createElement('div', { className: 'training-history-component' },
        ...
        // Configuration Modal
        showConfigModal && React.createElement(ConfigurationModal, {
